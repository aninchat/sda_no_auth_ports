...
def dnac_get_all_devices(token, dnac_ip_address):
    ...
    for network_device in stripped_response:
        temp_dict = {}
        temp_dict['hostname'] = network_device['hostname']
        temp_dict['ip_address'] = network_device['managementIpAddress']
        temp_dict['id'] = network_device['id']
        temp_dict['family'] = network_device['family']
...
def get_device_config(dnac_ip_address, token, device_list):
    ...
    for device in device_list:
        if device['family'] == 'Unified AP' or device['family'] == 'Wireless Controller':
            continue
        temp_dict = {}
        full_url = "https://" + dnac_ip_address + url + device['id'] + "/config"
